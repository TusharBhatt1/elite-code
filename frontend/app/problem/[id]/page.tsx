...
function buildSnippet(language: Language, fn: ProblemFunction): string {
...
function ConsoleView({
  ...
  if (!showResults || !results) {
    return (
      <pre className="flex-1 overflow-auto whitespace-pre-wrap bg-muted p-4 font-mono text-sm">
        {message}
      </pre>
    );
  }
        ...
        >
          ...
          <div className="space-y-1 font-mono text-xs">
            ...
            <p>
              ...
              <span className={tc.passed ? '' : 'font-semibold text-red-700'}>
...
export default function ProblemPage() {
  ...
  const isCodeRunning =
    !!runId &&
    (!runResult || runResult.status === RunStatus.PENDING);
  ...
  useEffect(() => {
    ...
    const results = (submission.result?.results ?? []) as TestResult[];
    ...
    if (allPassed) {
      ...
      toast.error('Runtime error', {
        description: 'Your code threw an error.',
      });
    } else {
  ...
  useEffect(() => {
    ...
    if (runResult.status === RunStatus.FAILED) {
      ...
      toast.error('Run failed', {
        description: runResult.error ?? 'Execution failed.',
      });
    ...
    const results = (
      Array.isArray(runResult.result)
        ? runResult.result
        : (runResult.result as any)?.results ?? []
    ) as TestResult[];
  ...
  const handleRun = () => {
    ...
    runCode(
      ...
      },
      {
        ...
        onError: (error: any) => {
          const msg =
            error?.response?.data?.message ?? 'Failed to start run.';
  ...
  return (
    ...
    <ResizablePanelGroup>
      <ResizablePanel defaultSize={50} minSize={25}>
        <div className="h-full space-y-6 overflow-y-auto border-r p-6">
          <div>
